...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/23 13:22:44 by rmanfred          #+#    #+#             */
/*   Updated: 2020/06/23 13:22:50 by rmanfred         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*to_save[FD_SIZE];
	char		*buf;
	int			bytes_to_read;

	if ((fd < 0) || !line || (BUFFER_SIZE < 1) || (fd >= FD_SIZE))
		return (-1);
	if (!to_save[fd] && !(to_save[fd] = ft_strdup("")))
		return (-1);
	...
	bytes_to_read = 1;
	while ((!(ft_strchr(to_save[fd], '\n')))
		&& ((bytes_to_read = read(fd, buf, BUFFER_SIZE)) > 0))
		if (!(to_save[fd] = to_join(bytes_to_read, buf, to_save[fd])))
		{
			free(buf);
			return (-1);
		}
	free(buf);
	return (check_result(bytes_to_read, line, &to_save[fd]));
}
